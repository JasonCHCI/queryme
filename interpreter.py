    ...
    if all(tokens.count(keyWord) == 1 for keyWord in ('SELECT', 'FROM')):
        ...
        if tokens.count('WHERE') == 1:
            ...
            if iselect < ifrom - 1 and ifrom < iwhere - 1 and iwhere < len(tokens) - 1:
                return True, ''.join(tokens[iselect + 1:ifrom]), tokens[ifrom + 1:iwhere], \
    ...
    if all(r in tables for r in relations):
        ...
        for a in attributes:
            ...
            if len(att) > 2:
                ...
                if att[0] not in schemas:
                    return 3
                elif att[1] not in schemas[att[0]]:
                    return 1
                else:
                    continue
            else:
            ...
            # if A is attribute,find data type. Otherwise return
                ...
                for table in tables:
                    if a in schemas[table]:
                        ...
                        tableA = table
                        attA = a
                if typeA is not None or count != 1:
                    print typeA is None
                    print count != 1
